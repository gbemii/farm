...
from user.forms import UserForm, FarmForm, Investment_schemeForm, TransationsForm, Admin_user_profile_update_form
...
def transaction_update_view(request, pk, *args, **kwargs):
	...
	if form.is_valid():
		form.save()
		return redirect('/transdetails/pk/')
	
	context={ 'agent': agent, 'form': form, 'trans': trans
	}
...
def investment_update_view(request, pk, *args, **kwargs):
	...
	context={ 'agent': agent, 'form': form, 'scheme': scheme
	}
...
def admin_users_update_view(request, pk, *args, **kwargs):
	agent= request.user
	person= User.objects.get(id=pk)
	# user= User.objects.get(id=pk)


	form= Admin_user_profile_update_form(request.POST or None, instance= person)
	if form.is_valid():
		form.save()
		# return redirect('investment_list/')
